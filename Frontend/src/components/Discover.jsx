...
const Discover = () => {
    ...
    const data = [
        ...
        { id: 3, icon: <RiRadarFill className='text-3xl text-[#15a7e7] font-extrabold' />, title: 'Communications, Media, and Information Services', link: '/communications-media' },
        ...
        { id: 8, icon: <GiRobotGrab className='text-3xl text-[#15a7e7] font-extrabold' />, title: 'High Tech', link: '/high-tech' },
        { id: 9, icon: <LuCircleDollarSign className='text-3xl text-[#15a7e7] font-extrabold' />, title: 'Insurance', link: '/insurance' },
        { id: 10, icon: <GiDna2 className='text-3xl text-[#15a7e7] font-extrabold' />, title: 'Life Science', link: '/life-science' },
  ...
  <div className='xl:w-full w-full px-10 bg-white'>
    <h2 className='text-3xl font-bold text-black uppercase tracking-wide pt-5'>Discover Our Services</h2> {/* Enhanced styling */}
    <div className='grid grid-cols-1 xl:grid-cols-2 gap-5 pt-8'>
        {data.map((item) => (
            <div className='group' key={item.id}>
                <div 
                    className='flex gap-3 items-center xl:w-full relative py-3 border-b-2 border-b-gray-600 group-hover:border-b-black transition-all duration-300'
                >
                    {item.icon}
                    <p 
                    ...
                    <FaArrowRight 
                        className='text-[#15a7e7] absolute right-5 opacity-0 group-hover:opacity-100 group-hover:right-0 transition-all duration-300 cursor-pointer' 
                        onClick={() => handleNavigation(item.link)} 
                    />
                </div>
            </div>
